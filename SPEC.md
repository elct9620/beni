...
| instance variable | read a named instance variable — `nil` when unset — assign one in place, test its presence, remove one and yield its former value, or iterate over every set instance variable; the read and presence test never raise; the assignment surfaces an `Err` when the receiver is frozen or cannot hold instance variables; the removal yields the former value, distinguishes an absent variable (and a receiver that cannot hold instance variables) from one removed while holding `nil`, and surfaces an `Err` only when a receiver that can hold instance variables is frozen; the iteration hands each set variable's name as a typed symbol and its value to a closure that signals whether to continue or stop — stopping ends the walk before the remaining variables — visiting nothing for a receiver that holds no instance variables, dispatching no Ruby and surfacing no `Err`; mutating the receiver's instance variables from within the closure is unsupported, and a closure panic stops the walk and resurfaces on the Rust side once the walk unwinds, never crossing into mruby's frames |
