...
# configured integer width, so the host is linted against the repo's
# 32-bit validation archive and against mruby's upstream-default 64-bit
# one, and wasm32 against its own. Each needs its staged libmruby.a, so a
# missing archive fails here and names the command that produces it. A
# gate that skipped instead would read as a warning nobody has to act on,
# which is how an unlinted branch reaches a commit.
set -eu
