...
# =================================
#
# Temporary compile-verification chain for the beni crates, ported from
# kobako's Stage C tasks. `crates/beni-sys`'s build.rs auto-detects the
# vendored `libmruby.a` for the active target (host → build/host/lib,
...
# overrides that build.rs honors directly.
#
#   $ rake rust:check       — cargo check, host target (placeholder mode
#                             when libmruby.a is absent)
#   $ rake rust:test        — cargo test, host target; with mruby:build
...
#                             is not provisioned)
#   $ rake rust:verify      — mruby:build + the three tasks above; the
...
namespace :rust do
  ...
  desc "Full local compile verification: mruby:build + host check/test + wasm32 check"
  task verify: ["mruby:build", "rust:check", "rust:test", "rust:check:wasm"]
end
