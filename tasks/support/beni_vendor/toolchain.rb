...
module BeniVendor
  ...
  # constant in +BeniVendor+; the rake DSL loop in +vendor.rake+
  ...
  #                          +BeniVendor.base_url_for+ so test fixtures
  ...
  #   * +sha_key+          — symbol used by +BeniVendor.expected_sha256+
  #                          to look up the +BENI_VENDOR_<KEY>_SHA256+
  ...
  Toolchain = Data.define(
    ...
    def url
      "#{BeniVendor.base_url_for(base_url)}/#{tarball_name}"
    end

    # Local cache path for the downloaded tarball. Lives under
    # +BeniVendor::CACHE_DIR+ regardless of +VENDOR_DIR+ overrides
    # (the cache moves with the vendor tree).
    def tarball_path
      File.join(BeniVendor::CACHE_DIR, tarball_name)
    end
    ...
    def verify
      Checksum.new(tarball_path, BeniVendor.expected_sha256(sha_key)).verify_or_pin
    end
