...
| API coverage | `.api_coverage.yml` → `docs/api_coverage.md` | `rake api:coverage` diffs mruby's scanned C surface against the Rust layers. The sys tier and `#define` equivalences are derived; the manifest hand-curates four per-symbol states, so a symbol absent from all of them is API still owed and nothing else (see Principle 11). `rake api:priority` orders what is owed by what downstream calls — a Rust consumer's use ranks above any number of mrbgem uses, and `BENI_CONSUMER_PATHS` points the scan at a consumer checkout. |
