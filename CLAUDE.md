...
`beni-sys/build.rs` has two paths: bindgen against a discovered archive's headers, and the documentation build (`DOCS_RS` set) that stages the checked-in `src/bindings_docs.rs` into OUT_DIR and links nothing. Everything else panics naming what it looked for — no archive, no build. `rake docs:bindings` rewrites the checked-in file from an upstream-default mruby, and only on `x86_64-unknown-linux-gnu` — bindgen output carries the host (libc's `va_list` and struct names, the presym set mruby's platform headers decide), so another host would write a file the gate rejects. The mruby C API itself does not vary: measured across the two platforms, the `pub fn` set is identical, which is why the checks split in two — **every platform** runs `rust:check:docs`, proving the committed file still describes a surface the crate compiles against, and **the documentation host's platform alone** (CI ubuntu-latest) regenerates byte-for-byte, proving it is current. The first of those compiles before it renders: rustdoc type checks signatures and not function bodies, and a body is where a new `sys::` call — the way the bindings surface grows — appears, so a render alone answers the question it is asked with a yes it has not checked. That gate attaches its regenerated file as an artifact, which is how a host that cannot write one commits it.

CI (`.github/workflows/main.yml`) runs four lanes, split by whether the work needs a staged archive: **test** (Ruby matrix, default task), **lint** (fmt + the `DOCS_RS` documentation build, compiled then rendered + the assertion that any other archive-free build fails), **verify** (3 OS full `rust:verify`, plus clippy, the private-item doc gate, and the coverage and documentation-bindings freshness gates on one OS), **scenario** (consumer harnesses). Tarballs are deliberately not cached — the download path is itself under test.
