...
8. **`test/` holds unit tests; `test/scenarios/` holds consumer harnesses** — each a consumer-shaped Rakefile run through the gem's task surface alone (`scenario:setup` → `beni:build` → `scenario:verify`), excluded from the default glob (else the vendored mruby tree's own `*_test.rb` get swept in). New consumer-visible behavior gets a scenario, not a unit test that fakes the task layer. The Rust side splits the same way three times: `crates/beni-tests/` holds the wrapper's behavior suite, reaching it through public paths alone and always against a staged archive; a `#[cfg(test)]` module inside `crates/beni/` holds only what no consumer can observe (a private field, a `pub(crate)` helper); `test/scenarios/rust_consumer/` holds the chain from `beni:build` to a crate that links what it produced, and `test/scenarios/embedded_no_compiler/` the same chain for the posture where the archive carries no compiler gem and the crate turns beni's default features off. A new wrapper test goes to `beni-tests` unless it needs something private.
...
11. **The typed `beni` surface graduates only what is safe to use without VM-internal reasoning** — a stronger bar than "cannot cause UB". An operation reaches the safe surface only when the wrapper can encode its invariant (a lifetime, carrier, or runtime check); otherwise the honest form is `unsafe` — a typed `unsafe fn` on the `beni` surface when a typed shape can still carry the value with one caller-owned invariant unencoded, or a raw `beni::sys` binding when the value is VM-internal with no shape to add (call-frame indices, `mrb_context` / RBreak fields). One unsafe only for want of an unbuilt carrier graduates once built; a permanently VM-internal one stays in `sys`. Zeroing a consumer's `sys::` use is never the goal. Refines Principle 6; the typed-surface contract lives in SPEC (e.g. `Proc::call` / `as_break` graduated, the raw frame-index shims stayed in `sys`). Every graduation is recorded in the `.api_coverage.yml` manifest — add the C symbol with the public Rust item that binds it under `typed:` (a Rust-native construct with no 1:1 C API goes under `extensions:` instead); `rake api:coverage` regenerates `docs/api_coverage.md` from it. Three sibling sections carry the rest of the verdict, each entry naming what settles it: `subsumed:` where a graduated item already delivers the symbol's whole capability, `declined:` where no typed shape can carry it, `conditional:` where a build flag this ABI lacks gates it. `admitted:` sits beside all four rather than among them — a symbol from a header mruby marks internal to the library is not embedder API, so it enters neither side of the ratio; admit one only for the typed item that carries it, by copying its single declaration into `wrapper.h` rather than including that header. A symbol whose equivalence a `#define` already states is derived, never recorded. A capability merely awaiting a carrier belongs to none of them — leaving it unrecorded is what keeps "absent" meaning "still owed". Only record symbols reachable through a public interface: test-only or internal-only bindings stay out until they are genuinely graduated.

## Build Pipeline

The repo dogfoods its own gem: the Rakefile wires `Beni::Tasks` with the validation config `build_config/mruby.rb` (host + wasi targets, ABI-pinned with `MRB_INT32` + `MRB_WORDBOX_NO_INLINE_FLOAT`), while the gem's default stays mruby's untouched upstream config. `rake rust:verify` is the single local gate: `beni:build` → host `cargo check`/`test` → `rust:check:nodefault` (the beni crate alone with every capability feature off, named as one package because workspace feature unification would otherwise restore a sibling's defaults) → wasm32 `cargo check`/link → `rust:check:docs` (the documentation build, no archive) → `rust:test:default` (tests against an upstream-default mruby build, catching int-width coincidences the MRB_INT32 config masks).

`beni-sys/build.rs` has two paths: bindgen against a discovered archive's headers, and the documentation build (`DOCS_RS` set) that stages `src/bindings_docs.rs` into OUT_DIR and links nothing. Everything else panics naming what it looked for — no archive, no build. `rake docs:bindings` writes that file from an upstream-default mruby, and nothing tracks it — `.gitignore` covers it and `include` in beni-sys's manifest is what puts it in the published package anyway, since the default package list is git's. Generated where it is read, so it cannot go stale and needs no freshness gate: `rust:check:docs` generates before it compiles, and compiles before it renders, because rustdoc type checks signatures and not function bodies — and a body is where a new `sys::` call, the way the bindings surface grows, appears. The host only matters for the copy a package carries, which docs.rs reads on `x86_64-unknown-linux-gnu`: `docs:bindings:release` refuses anywhere else, and the release workflow generates there and asserts the package listing carries the file, because `cargo package` would otherwise leave it out silently and only docs.rs would notice.

CI (`.github/workflows/main.yml`) runs four lanes, split by whether the work needs a staged archive: **test** (Ruby matrix, default task), **lint** (fmt + the assertion that an archive-free build fails), **verify** (3 OS full `rust:verify` — which is where the documentation build lives, its bindings needing an mruby to generate — plus clippy, the private-item doc gate, and the coverage freshness gate on one OS), **scenario** (consumer harnesses). What a real vendor download proves is that every `WASI_SDK_PLATFORM` detection branch picks the right tarball, so verify does one per build platform and caches nothing; a scenario repeating a platform verify already covered caches its tarballs instead, keyed on the pinned versions so a bump downloads for real. A cache hit skips the fetch alone — every checksum is still recomputed on the way in.
