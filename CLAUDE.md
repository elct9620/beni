...
4. **The compile-flags sidecar is the only ABI alignment channel.** `beni-sys` parses `libmruby.flags.mak` next to each archive; never hard-code ABI defines in the crates, and never let a staged archive without its sidecar fall back to guessing. The `beni` crate learns the configured integer and float widths from nothing but the metadata `beni-sys` reads out of the bindings it generates or stages — the `mrb_int64` and `mrb_float32` cfgs it gates the numeric conversions on. Bindings declaring no float at all (MRB_NO_FLOAT) stop the build rather than reaching a surface that converts floats. The gem ships no config template — `beni:config` copies the configured version's upstream default config from the staged mruby source; `build_config/mruby.rb` is the repo's own validation config (the generate-then-edit consumer posture, kept committed).

5. **Follow language community conventions via tooling.** Ruby: Rubocop + Steep; Rust: `cargo fmt` + `cargo clippy -D warnings` (also against every staged ABI — the upstream-default 64-bit integer archive and the MRB_USE_FLOAT32 one — since the crates compile a different set of numeric conversions under each configured width, and under `--target wasm32-wasip1` when the wasi archive is staged) + `cargo doc -D warnings --document-private-items`. All run via PostToolUse/Stop hooks and block on failure. When a cop or lint fires, shrink the code to fit the tool — don't widen `.rubocop.yml` exclusions or add `#[allow]`. Tool-vs-tool conflicts are the one justified widening: `Style/DataInheritance` is disabled because ruby/rbs documents `class X < Data.define(...)` as the Steep-friendly form.
...
- **Integer conversions are width-gated, not feature-gated.** Unlike a capability, the configured integer width is an ABI fact, so it is read rather than declared: `beni/build.rs` turns `DEP_MRUBY_DEFINES_MRB_INT64` into `mrb_int64`, and an integer type converts only where every value it holds fits. Width-dependent conversions stay lint-clean under both widths by spelling the target as `sys::mrb_int` (or as an identity per width) — clippy flags a conversion only when a concrete target type equals its source.
