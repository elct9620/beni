...
11. **The typed `beni` surface graduates only what is safe to use without VM-internal reasoning.** `beni-sys` binds the whole mruby header surface; the typed layer lifts a capability out of the unsafe `beni::sys` escape hatch only when a caller can use it *correctly* without knowing mruby's internal structure — a stronger bar than "cannot cause UB." An operation whose correctness depends on VM internals (call-frame layout, RBreak topology, raw `mrb_context` fields) stays behind `beni::sys`, where `unsafe` is the honest signal that the caller owns invariants the compiler cannot check; a safe-looking wrapper over it would misrepresent its sharpness, and closing a consumer's `sys::` use to zero is never the goal — honesty is. This refines Principle 6: the beni surface grows toward magnus, but only with operations that are genuinely safe. Worked example: `Proc::call` / `Value::as_break` / `Break::value` (what an escaped block value *is*) graduated to the typed surface; `mrb_current_ci_index_func` / `mrb_break_ci_index_func` (*where* a break unwinds in the internal call-frame array, needed only to classify a non-local exit) stayed in `beni::sys`.
