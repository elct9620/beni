...
mruby 4.0.0 · sys detection: linked (parsed bindings.rs) · 467 scanned · outside the measure: 30 declined, 7 conditional
...
| `mrb_exc_new` | fn | ✅ | ✅ | `ExceptionClass::exc_new`, also via `Error::new` (see Error extension) |
...
| `mrb_get_argc` | fn | ✅ | ✅ | `Mrb::argc` — the call's argument count, a non-empty keyword hash counted as one trailing positional as magnus's argument slice holds it; the positionals alone are a `scan_args` splat beside its keyword part |
...
| `mrb_get_args_a` | fn | ✅ | ❌ |  |
| `mrb_get_argv` | fn | ✅ | ✅ | `Mrb::argv` — a copy of the call's arguments, valid across a VM re-entry, the companion to `Mrb::argc`, a non-empty keyword hash its last value |
| `mrb_get_mid` | fn | ✅ | ❌ |  |
...
| `mrb_range_beg_len` | fn | ✅ | ✅ | `Range::beg_len` — the normalized slice a Range covers of a collection of a given length (Ruby's `Array#[range]` / `String#[range]`); returns the three-way `RangeBegLen` outcome (in-range, out-of-range, non-Range mismatch), raising TypeError on a non-integer bound, caught by exception protection into `Err` |
...
| `sys::AsRawValue / sys::FromRawValue / sys::AsRawId / sys::FromRawId` | The raw seam as magnus's `rb_sys` traits carry it, beside the raw bindings: the safe read of a raw `mrb_value` out of any `ReprValue` handle or of the id out of an `Id`, and the `unsafe` crossings back into `Value` and `Id`. A class pointer has no crossing of its own, as in magnus: a class handle's raw form is its value, and a raw pointer crosses boxed as a value through the checked downcast. Binds no C symbol. |
