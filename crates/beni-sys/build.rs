...
// host has nowhere to stage an archive and never links, so the checked-in
...
// stage an archive, and never links. That build copies the checked-in
// `src/bindings_docs.rs` — generated by `rake docs:bindings` from an
// mruby built with mruby's own default config — into `$OUT_DIR` under
// the name every other build's bindings take, so `src/lib.rs` includes
// one path and the crate carries no second shape.
//
// Idempotency
// -----------
...
/// Put the checked-in documentation bindings where `src/lib.rs`
